...
This document provides a detailed line-by-line explanation of the Memory Requests Enforcement Rego policy for Kubernetes Deployments.

## 📋 Policy Overview

The policy validates that all Kubernetes Deployment containers (including initContainers and ephemeralContainers) have memory requests configured to help Kubernetes scheduler make better placement decisions.
...
```bash
opa eval --data policies/memory-requests-enforcement.rego \
        --input test-data/invalid-deployment-missing-memory-request.json \
        "data.wiz.result"
# Expected: "fail"
```
