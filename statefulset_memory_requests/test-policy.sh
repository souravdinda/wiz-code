...
# Test script for StatefulSet Memory Requests Enforcement Policy
# This script tests the Rego policy against various test cases

set -e

POLICY_FILE="policies/memory-requests-enforcement.rego"
TEST_DIR="test-data"
...
echo "Testing StatefulSet Memory Requests Enforcement Policy"
echo "========================================"
echo ""
...
test_case "valid-statefulset.json" "ALLOWED" "Statefulset with memory request"

# Invalid cases (should be DENIED/fail)
test_case "invalid-statefulset-missing-memory-request.json" "DENIED" "Statefulset missing memory request"
test_case "invalid-statefulset-no-resources.json" "DENIED" "Statefulset with no resources section"
