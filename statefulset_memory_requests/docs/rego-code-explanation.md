...
This document provides a detailed line-by-line explanation of the Memory Requests Enforcement Rego policy for Kubernetes StatefulSets.

## 📋 Policy Overview

The policy validates that all Kubernetes StatefulSet containers (including initContainers and ephemeralContainers) have memory requests configured to help Kubernetes scheduler make better placement decisions.
...
# This rule checks if StatefulSet containers have memory requests defined
...
**Key Points:**
- Uses `input.object.spec.template.spec[containerPaths[]][]` to iterate through all container types
...
```bash
opa eval --data policies/memory-requests-enforcement.rego \
...
```bash
opa eval --data policies/memory-requests-enforcement.rego \
        --input test-data/invalid-statefulset-missing-memory-request.json \
        "data.wiz.result"
# Expected: "fail"
```
...
This policy ensures that all containers in Kubernetes StatefulSets have memory requests configured. The policy uses a fail-safe approach with `default result = "fail"` and comprehensively checks all container types using set comprehension to compare counts of containers with memory requests against all containers.
