...
This document provides a detailed line-by-line explanation of the Memory Limits Enforcement Rego policy for Kubernetes Deployments.

## 📋 Policy Overview

The policy validates that all Kubernetes Deployment containers (including initContainers and ephemeralContainers) have memory limits configured to prevent resource contention and unpredictable performance.
...
# This rule checks if Deployment containers have memory limits defined
...
```bash
opa eval --data policies/memory-limits-enforcement.rego \
        --input test-data/invalid-deployment-missing-memory-limit.json \
        "data.wiz.result"
# Expected: "fail"
```
