...
This document provides a detailed line-by-line explanation of the CPU Limits Enforcement Rego policy for Kubernetes StatefulSets.

## 📋 Policy Overview

The policy validates that all Kubernetes StatefulSet containers (including initContainers and ephemeralContainers) have CPU limits configured to prevent resource contention and unpredictable performance.
...
currentConfiguration := "CPU limits are not set for containers in the statefulset"
expectedConfiguration := "CPU limits should be set for all containers in the statefulset"
```
...
```bash
opa eval --data policies/cpu-limits-enforcement.rego \
...
```bash
opa eval --data policies/cpu-limits-enforcement.rego \
        --input test-data/invalid-statefulset-missing-cpu-limit.json \
        "data.wiz.result"
# Expected: "fail"
```
