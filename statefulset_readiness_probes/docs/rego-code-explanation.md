...
This document provides a detailed line-by-line explanation of the Readiness Probes Enforcement Rego policy for Kubernetes StatefulSets.

## 📋 Policy Overview

The policy validates that Kubernetes StatefulSet containers (including initContainers and ephemeralContainers) have readiness probes configured to ensure containers are ready to serve traffic before receiving requests.
...
# Check if StatefulSet has custom readiness probes defined for containers
...
currentConfiguration := "StatefulSet containers do not have readiness probes configured"
expectedConfiguration := "StatefulSet containers should have readiness probes configured to ensure proper health checking"
```
...
```
**What it does:** 
- Iterates through all containers in `input.object.spec.template.spec.containers`
...
### Test with Valid StatefulSet
```bash
opa eval --data policies/readiness-probe-enforcement.rego \
        --input test-data/valid-statefulset-with-readiness.json \
        "data.wiz.result"
# Expected: "pass"
```

### Test with Invalid StatefulSet
```bash
opa eval --data policies/readiness-probe-enforcement.rego \
        --input test-data/invalid-statefulset-missing-readiness.json \
        "data.wiz.result"
# Expected: "fail"
```
...
This policy ensures that Kubernetes StatefulSets have readiness probes configured for at least one container type (containers, initContainers, or ephemeralContainers). The policy uses a fail-safe approach with `default result = "fail"` and comprehensively checks all container types using an else chain pattern.
