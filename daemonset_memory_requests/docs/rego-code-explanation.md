...
This document provides a detailed line-by-line explanation of the Memory Requests Enforcement Rego policy for Kubernetes DaemonSets.

## 📋 Policy Overview

The policy validates that all Kubernetes DaemonSet containers (including initContainers and ephemeralContainers) have memory requests configured to help Kubernetes scheduler make better placement decisions.
...
# This rule checks if DaemonSet containers have memory requests defined
...
```bash
opa eval --data policies/memory-requests-enforcement.rego \
        --input test-data/invalid-daemonset-missing-memory-request.json \
        "data.wiz.result"
# Expected: "fail"
```
