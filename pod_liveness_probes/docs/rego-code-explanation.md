...
This document provides a detailed line-by-line explanation of the Liveness Probes Enforcement Rego policy for Kubernetes Pods.

## 📋 Policy Overview

The policy validates that all Kubernetes Pod containers (including initContainers and ephemeralContainers) have liveness probes configured to ensure containers are restarted when they become unhealthy.
...
```bash
opa eval --data policies/probes-enforcement.rego \
        --input test-data/invalid-pod-missing-liveness.json \
        "data.wiz.result"
# Expected: "fail"
```
