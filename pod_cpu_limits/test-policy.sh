...
# Test script for CPU Limit Enforcement Policy
# This script tests the Rego policy against various test cases

set -e

POLICY_FILE="policies/cpu-limit-enforcement.rego"
TEST_DIR="test-data"
...
echo "Testing CPU Limit Enforcement Policy"
echo "======================================"
echo ""
...
test_case "valid-pod.json" "ALLOWED" "Pod with CPU limit"
test_case "valid-pod-multi-container.json" "ALLOWED" "Pod with multiple containers, all have CPU limits"

# Invalid cases (should be DENIED/fail)
test_case "invalid-pod.json" "DENIED" "Pod missing CPU limit (has requests only)"
test_case "invalid-pod-no-resources.json" "DENIED" "Pod with no resources section"
test_case "invalid-pod-missing-cpu-limit.json" "DENIED" "Pod missing CPU limit (has memory limit)"
test_case "invalid-pod-one-container-missing-limit.json" "DENIED" "Pod with one container missing CPU limit"
