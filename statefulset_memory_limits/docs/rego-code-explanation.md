...
This document provides a detailed line-by-line explanation of the Memory Limits Enforcement Rego policy for Kubernetes StatefulSets.

## 📋 Policy Overview

The policy validates that all Kubernetes StatefulSet containers (including initContainers and ephemeralContainers) have memory limits configured to prevent resource contention and unpredictable performance.
...
```bash
opa eval --data policies/memory-limits-enforcement.rego \
        --input test-data/invalid-statefulset-missing-memory-limit.json \
        "data.wiz.result"
# Expected: "fail"
```
