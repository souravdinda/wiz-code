...
# This rule checks if Kubernetes daemonsets have both liveness and readiness probes configured for all containers
# DaemonSets without probes can lead to unhealthy containers serving traffic or containers not being restarted when needed
default result = "pass"

currentConfiguration := sprintf("DaemonSet '%s' containers without liveness or readiness probes: %v", [input.metadata.name, containers_without_probes])
