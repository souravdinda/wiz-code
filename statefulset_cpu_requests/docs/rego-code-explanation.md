...
This document provides a detailed line-by-line explanation of the CPU Requests Enforcement Rego policy for Kubernetes StatefulSets.

## 📋 Policy Overview

The policy validates that all Kubernetes StatefulSet containers (including initContainers and ephemeralContainers) have CPU requests configured to help Kubernetes scheduler make better placement decisions.
...
- ❌ **Consequence**: Init containers and ephemeral containers without CPU requests would be missed
...
result = "pass" {
  input.spec.template.spec.containers[].resources.requests.cpu
}
...
```bash
opa eval --data policies/cpu-request-enforcement.rego \
