...
This document provides a detailed line-by-line explanation of the Memory Limits Enforcement Rego policy for Kubernetes DaemonSets.

## 📋 Policy Overview

The policy validates that all Kubernetes DaemonSet containers (including initContainers and ephemeralContainers) have memory limits configured to prevent resource contention and unpredictable performance.
...
# This rule checks if DaemonSet containers have memory limits defined
...
```bash
opa eval --data policies/memory-limits-enforcement.rego \
        --input test-data/invalid-daemonset-missing-memory-limit.json \
        "data.wiz.result"
# Expected: "fail"
```
...
This policy ensures that all containers in Kubernetes DaemonSets have memory limits configured. The policy uses a fail-safe approach with `default result = "fail"` and comprehensively checks all container types using set comprehension to compare counts of containers with memory limits against all containers.
