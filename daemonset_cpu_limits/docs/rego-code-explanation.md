...
This document provides a detailed line-by-line explanation of the CPU Limits Enforcement Rego policy for Kubernetes DaemonSets.

## 📋 Policy Overview

The policy validates that all Kubernetes DaemonSet containers (including initContainers and ephemeralContainers) have CPU limits configured to prevent resource contention and unpredictable performance.
...
```bash
opa eval --data policies/cpu-limits-enforcement.rego \
        --input test-data/invalid-daemonset-missing-cpu-limit.json \
        "data.wiz.result"
# Expected: "fail"
```
