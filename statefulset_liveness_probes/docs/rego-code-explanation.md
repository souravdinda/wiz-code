...
This document provides a detailed line-by-line explanation of the Liveness Probes Enforcement Rego policy for Kubernetes StatefulSets.

## 📋 Policy Overview

The policy validates that all Kubernetes StatefulSet containers (including initContainers and ephemeralContainers) have liveness probes configured to ensure containers are restarted when they become unhealthy.
...
opa eval --data policies/probes-enforcement.rego \
        --input test-data/valid-statefulset-with-probes.json \
        "data.wiz.result"
# Expected: "pass"
```
...
```bash
opa eval --data policies/probes-enforcement.rego \
        --input test-data/invalid-statefulset-missing-liveness.json \
        "data.wiz.result"
# Expected: "fail"
```
