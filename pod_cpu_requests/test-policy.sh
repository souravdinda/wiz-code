...
# Test script for CPU Request Enforcement Policy
# This script tests the Rego policy against various test cases

set -e

POLICY_FILE="policies/cpu-request-enforcement.rego"
TEST_DIR="test-data"
...
echo "Testing CPU Request Enforcement Policy"
echo "======================================"
echo ""
...
test_case "valid-pod.json" "ALLOWED" "Pod with CPU request"
test_case "valid-pod-multi-container.json" "ALLOWED" "Pod with multiple containers, all have CPU requests"

# Invalid cases (should be DENIED/fail)
test_case "invalid-pod.json" "DENIED" "Pod missing CPU request (has limits only)"
test_case "invalid-pod-no-resources.json" "DENIED" "Pod with no resources section"
test_case "invalid-pod-one-container-missing-request.json" "DENIED" "Pod with one container missing CPU request"
