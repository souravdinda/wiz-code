...
This document provides a detailed line-by-line explanation of the CPU Limits Enforcement Rego policy for Kubernetes Deployments.

## 📋 Policy Overview

The policy validates that all Kubernetes Deployment containers (including initContainers and ephemeralContainers) have CPU limits configured to prevent resource contention and unpredictable performance.
...
currentConfiguration := "CPU limits are not set for containers in the deployment"
expectedConfiguration := "CPU limits should be set for all containers in the deployment"
```
...
```bash
opa eval --data policies/cpu-limit-enforcement.rego \
        --input test-data/invalid-deployment-missing-cpu-limit.json \
        "data.wiz.result"
# Expected: "fail"
```
