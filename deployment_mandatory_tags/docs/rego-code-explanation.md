...
This document provides a detailed line-by-line explanation of the Mandatory Tags Enforcement Rego policy for Kubernetes Deployments.

## 📋 Policy Overview

The policy validates that all Kubernetes Deployment resources have mandatory labels (`environment`, `owner`, `project`, `team`) configured with non-empty values. This ensures proper resource tagging for governance, cost allocation, and compliance.
...
### Test with Invalid Deployment (Missing Labels)
```bash
opa eval --data policies/mandatory-tags-enforcement.rego \
...
### Test with Invalid Deployment (Empty Values)
```bash
opa eval --data policies/mandatory-tags-enforcement.rego \
