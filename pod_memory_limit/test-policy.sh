...
# Test script for Memory Limit Enforcement Policy
# This script tests the Rego policy against various test cases

set -e

POLICY_FILE="policies/memory-limit-enforcement.rego"
TEST_DIR="test-data"
...
echo "Testing Memory Limit Enforcement Policy"
echo "========================================"
echo ""
...
test_case "valid-pod.json" "ALLOWED" "Pod with memory limit"
test_case "valid-pod-multi-container.json" "ALLOWED" "Pod with multiple containers, all have memory limits"

# Invalid cases (should be DENIED/fail)
test_case "invalid-pod.json" "DENIED" "Pod missing memory limit (has CPU limit only)"
test_case "invalid-pod-no-resources.json" "DENIED" "Pod with no resources section"
test_case "invalid-pod-one-container-missing-limit.json" "DENIED" "Pod with one container missing memory limit"
